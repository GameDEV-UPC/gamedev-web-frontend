...
function NoiseButton({ options }: NoiseButtonProps) {
    ...
    useEffect(() => {
        ...
        const animate = (elements: Element[][], isChecked: boolean) => {
            ...
            const options = { duration: 0.8, ease: "elastic.out(1, 0.3)", stagger: 0.01, overwrite: true };
        ...
        const inputs = Array.from(containerRef.current.querySelectorAll("input[type='radio']")) as HTMLInputElement[];
        inputs.forEach((input) => input.addEventListener("change", () => handleRadioChange(input)));
